...
```{bash, eval=FALSE}
# You already ran this in the Bash terminal - but it's super nice that you can run it here too!
...
print(a_list[0]) # zero-indexing - takes the first number!!!
print(a_list[1])

new_list = [0, 1, 2, 3, 4, 5]
print(new_list[0:3])  # slicing
# IMPORTANT!!! Python never includes the last number you index - you might specify you want 0:3, but that gives you positions 0, 1 and 2. 
```

Indentation and loops:
```{python}
# indentation (use tabulation) controls scope of control variables
for index in range(0, 5): # indentation (use tabulation) controls scope of control variables 
    ...
    if index == 0: # remember the colon
        ...
        value += index
    print(value)
# It's not gonna print shit if there's nothing breaking the tabulation!
```
...
x = np.arange(10) # generate 9 values of x

y = 2 * x # the values of y are just double the values of x
y = y.astype(float) # or y = float(y)

n_samples = len(y)
y += np.random.normal(loc = 0, scale = 1, size = n_samples) # add error

X = np.zeros(shape = (n_samples, 2)) # create matrix of X; here we just establish the structure
X[:, 0] = x ** 0 # the first column is the values of x to the power of 0 (i.e. a column of 1s)
X[:, 1] = x ** 1 # the second column is the values of x to the power of 1 (i.e. the values themselves)

beta = .linalg.inv(np.transpose(X) @ X) @ np.transpose(X) @ y #find coefficients through ordinary least squares
print(beta)

```

2) Create a model matrix, $X$ that estimates, $\hat\beta$ the means of the three sets of observations below, $y_1, y_2, y_3$ (Exercise 1.2) (ANOVA!)
