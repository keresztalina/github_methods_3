...
for (i in 1:29) {
  ...
  plot <- ggplot(loopdf, aes(x = target.contrast, y = inv)) +
    ...
    scale_color_manual(labels = c("No-pool function"), values = c("red")) +
...
for (i in 1:29) {
  ...
  plot <- ggplot(loopdf, aes(x = target.contrast, y = inv)) +
    geom_point(aes(y = inv, color = 'red')) +
    geom_point(aes(y = pooled_inv, color = 'blue')) +
    scale_color_manual(labels = c("Partial-pooling function", "No-pool function"), values = c("blue", "red")) +
    guides(color=guide_legend("Functions")) + 
...
#AIC
AIC(model_3.3a, model_3.3b, model_3.3c, model_3.3d, test = 'LR')
```
