---
title: "Portfolio Exercise 2.1, Methods 3, 2021, autumn semester"
...
pacman::p_load(tidyverse, lme4, dfoptim, MuMIn, boot)
```
...
- *trial.type*: A staircase procedure was performed before the actual experiment procedure in order to find each participant's perceptual threshold. Since there are two categories of trial available, and they are not along a continuum, the variable should be encoded as a *factor*.
...
**i. comment on these**
Based on a visual inspection, the residuals are clearly not normally distributed (they do not approximate the line), and they are heavily skewed.
...
The residuals appear to better approximate a normal distribution after log-transformation, although they still exhibit a certain amount of skewness (much less than the non-transformed residuals).
...
# Build models
model_2.1 <- lme4::lmer(rt.obj ~ task + (1 | subject), 
                        data = exp, 
                        REML = FALSE)
```
...
_Subject_ has to be included as a random effect as it represents sub-groups that significantly influence the outcome and make the data non-independent. 
...
The model shows that reaction times are highest for pairs tasks (α = 1.1201), with somewhat lower reaction times for singles tasks (β = -0.1915) and quadruplet tasks (β = -0.1532). 
...
model_2.2 <- lmerTest::lmer(rt.obj ~ pas * task + (1 | subject), 
                            data = exp, 
                            REML = FALSE)
```
...
```{r}
model_2.3 <- lmerTest::lmer(rt.obj ~ task + 
                              (1 | subject) + 
                              (1 | cue) + 
                              (1 | trial) + 
                              (1 | target.type) + 
                              (1 | pas), 
                            data = exp, 
                            REML = FALSE) # this model still converges
model_2.4 <- lmerTest::lmer(rt.obj ~ task + 
                              (1 | subject) + 
                              (1 | cue) + 
                              (1 | trial) + 
                              (1 | target.type) + 
                              ...
                              (1 | task), 
                            data = exp, 
                            REML = FALSE) # this model gives a singular fit
```

We succeeded at adding 5 group intercepts before singularity issues happened.
...
```{r}
# I take the model with singularity issues from the previous exercise
print(VarCorr(model_2.4), comp = 'Variance')
```
...
```{r}
model_3.1 <- lme4::glmer(count ~ pas * task + (1 + pas | subject), 
                   data = data.count, 
                   family = 'poisson', 
                   control = glmerControl(optimizer = "bobyqa")) # optimizer is used due to convergence issues
```

**i. which family should be used?**

A Poisson-distribution should be used as we are investigating count data.
    
**ii. why is a slope for _pas_ not really being modeled?**

_pas_ is encoded as factor, i.e. a categorical variable, meaning that it is assumed not to lie along a continuum (the differences between the levels are not incremental) and therefore has to be analyzed separately per level.
...
```{r}
model_3.2 <- lme4::glmer(count ~ pas + task + (1 + pas | subject), 
                    data = data.count, 
                    family = 'poisson', 
...
```{r}
# AIC comparison:
AIC(model_3.1, model_3.2)

# Residual variance:
c(sum(residuals(model_3.1)^2),
  sum(residuals(model_3.2)^2)
)
...
Model _model_3.1_, including the interaction between _pas_ and _task_ along with their main effects, has the lower residual variance, residual standard deviation and AIC score.
...
normal <- fixef(model_3.1)
exponentiated <- exp(fixef(model_3.1))
poisson <- as.data.frame(cbind(normal, exponentiated))

#If the value is negative, it implies a decrease, and the exponentiated value has to be subtracted from 1
poisson$corrected <- ifelse((poisson$normal < 0), 1 - poisson$exponentiated, poisson$exponentiated)
poisson$direction <- ifelse((poisson$normal < 0), "decrease", "increase")

poisson
```

As a baseline, i.e. for when the PAS rating is "1" and the task is "pairs", ~56.58% are so rated. It appears that there is an increasingly smaller probability for participants to rate a trial as a PAS 2 (~2.34%), PAS 3 (~40.01%) or PAS 4 (~53.83%) compared to a PAS 1. Participants also appear less likely to encounter trials with a "singles" task (~20.62%) and more likely to encounter trials with a "quadruplets" task (~12.18%) compared to a "pairs" task.
...
subject5 = data.count %>% 
  filter(subject == '5')
subject16 = data.count %>% 
  filter(subject == '16')
subject20 = data.count %>% 
  filter(subject == '20')

# Predict & exponentiate for them
pred1 = predict(model_3.1, newdata = subject1)
subject1$prediction = expm1(pred1)
pred5 = predict(model_3.1, newdata = subject5)
subject5$prediction = expm1(pred5)
pred16 = predict(model_3.1, newdata = subject16)
subject16$prediction = expm1(pred16)
pred20 = predict(model_3.1, newdata = subject20)
subject20$prediction = expm1(pred20)

# Get the subjects into one df
df_3.1 = rbind(subject1, subject5, subject16, subject20)

# Plot
ggplot(data = df_3.1, aes(x = pas, y = prediction, fill = task)) +
  geom_col(width = 0.75) +
  facet_wrap(~subject) +
...
No dataset was specified for this exercise, but it made most sense to us to use the data for the experimental trials.
```{r}
model_3.3a <- lme4::glmer(correct ~ task + (1 | subject), 
                          family = 'binomial', 
                          data = exp)
summary(model_3.3a)$coefficients
```

**i. does _task_ explain performance?**

_task_ appears to significantly modulate performance when it is _singles_ compared to _doubles_ (β = 0.16603, SE = 0.05217, p < .05), but not when it is _quadruplets_ compared to _doubles_.

**ii. add _pas_ as a main effect on top of _task_ - what are the consequences of that?**  
```{r}
model_3.3b <- lme4::glmer(correct ~ task + pas + (1 | subject), 
                          family = 'binomial', 
                          data = exp)
summary(model_3.3b)$coefficients
```

When taking both _pas_ and _task_ as predictor variables, _task_ no longer significantly explains performance, but _pas_ does (PAS 2: β = 0.88735, SE = 0.05414, p < .001; PAS 3: β = 1.87054, SE = 0.07482, p < .001; PAS 4: β = 2.88685, SE = 0.10240, p < .001).

**iii. now fit a multilevel model that models _correct_ as dependent on _pas_ with a unique intercept for each _subject_**
```{r}
model_3.3c <- lme4::glmer(correct ~ pas + (1 | subject), family = 'binomial', data = exp)
summary(model_3.3c)$coefficients
```

_pas_ significantly explains performance, (PAS 2: β = 0.88638, SE = 0.05407, p < .001; PAS 3: β = 1.86891, SE = 0.07462, p < .001; PAS 4: β = 2.88198, SE = 0.10169, p < .001).

**iv. finally, fit a model that models the interaction between _task_ and _pas_  and their main effects**  
```{r}
model_3.3d <- lme4::glmer(correct ~ task * pas + (1 | subject), family = 'binomial', data = exp)
summary(model_3.3d)
```

It still only seems to be _pas_ that significantly modulates accuracy, (PAS 2: β = 0.91504, SE = 0.08891, p < .001; PAS 3: β = 1.82975, SE = 0.12090, p < .001; PAS 4: β = 2.83307, SE = 0.17329, p < .001)

**v. describe in your words which model is the best in explaining the variance in accuracy**  
```{r}
#Residual variance:
c(sum(residuals(model_3.3a)^2),
  sum(residuals(model_3.3b)^2),
  sum(residuals(model_3.3c)^2),
  sum(residuals(model_3.3d)^2)
)

#AIC
AIC(model_3.3a, model_3.3b, model_3.3c, model_3.3d)
```

_model_3.3c_ appears to be the most suitable model. It has the lowest residual variance and AIC score, and the addition of multiple main effects or of interaction effects did not significantly improve the model's performance, and unnecessary added complexity may result in overfitting. PAS seems to account for most of the variance in the dataset.  
