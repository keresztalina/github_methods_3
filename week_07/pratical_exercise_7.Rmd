...
1) Create a function, $f(x)$ that squares its input. This is our __true__ function  (i)
```{python}
def truefunction(x):
  y = x ** 2
  return(y)
```
    i. generate data, $y$, based on an input range of [0, 6] with a spacing of 0.1. Call this $x$ (i)
```{python}
x = np.arange(start = 0, stop = 6, step = 0.1)
y_true = truefunction(x)
```
    ii. add normally distributed noise to $y$ with $\sigma=5$ (set a seed to 7 `np.random.seed(7)`) to $y$ and call it $y_{noisy}$ (i)
```{python}
np.random.seed(7) # for reproducibility
n_samples = len(y_true)
y_noisy = y_true + np.random.normal(loc = 0, scale = 1, size = n_samples) # add error; loc is mean, scale is sd
print(y_noisy)
```
    iii. plot the true function and the generated points (i)
```{python}
plt.figure() # create new figure
plt.plot(x, y_noisy, 'b.') # true function; 'b' refers to blue, '.' refers to dots
plt.plot(x, y_true, 'r-') # 'r' refers to red, '-' refers to line
plt.xlabel('x')
plt.ylabel('y')
...
plt.show()
```
2) Fit a linear regression using `LinearRegression` from `sklearn.linear_model` based on $y_{noisy}$ and $x$ (see code chunk below associated with Exercise 1.2) (i)
```{python}
regressor = LinearRegression() # making the function an object so we can append stuff to it
x_reshaped = x.reshape(-1, 1)
fit = regressor.fit(x_reshaped, y_noisy) 
```
    i. plot the fitted line (see the `.intercept_` and `.coef_` attributes of the `regressor` object) on top of the plot (from 1.1.iii) (i)
    ```{python}
intercept = fit.intercept_ 
slope = fit.coef_

plt.figure()
plt.plot(x, y_true, 'r-') # true function
plt.plot(x, slope * x + intercept, 'k-') # estimated function
plt.plot(x, y_noisy, 'b.') # generated points 
plt.xlabel('x')
plt.ylabel('y')
plt.title('True function, linear fit and generated points')
plt.legend(['True function', 'Estimated function', 'Generated points'])
plt.show()
```
    ii. now run the code chunk below associated with Exercise 1.2.ii - what does X_quadratic amount to? (i)
    ```{python}
quadratic = PolynomialFeatures(degree = 2)
X_quadratic = quadratic.fit_transform(x.reshape(-1, 1))
# print(X_quadratic)
# it's the design matrix we used to calculate OLS in the previous exercise, but a third column was added to represent 'x^2' in (ax^2 + bx + c)
```
    iii. do a quadratic and a fifth order fit as well and plot them (on top of the plot from 1.2.i) (i once I googled what 'fifth order fit' meant)
    ```{python}
# quadratic fit
fit_2 = regressor.fit(X_quadratic, y_noisy)

# fifth order fit
fifth_order = PolynomialFeatures(degree = 5)
X_fifth_order = fifth_order.fit_transform(x.reshape(-1, 1))
fit_5 = regressor.fit(X_fifth_order, y_noisy)

# acquire fitted values
y_pred_2 = regressor.predict(X_quadratic)
y_pred_5 = regressor.predict(X_fifth_order)

# plot
plt.figure()
plt.plot(x, y_true, 'r-') # true function
plt.plot(x, slope * x + intercept, 'k-') # estimated function (linear)
plt.plot(x, y_noisy, 'b.') # generated points 
plt.plot(x, y_pred_2, 'm-')  # estimated function (quadratic)
plt.plot(x, y_pred_2, 'c-')  # estimated function (fifth order)
plt.xlabel('x')
plt.ylabel('y')
plt.title('True function, linear fit and generated points')
plt.legend(['True function', 'Linear estimate', 'Quadratic estimate', 'Fifth order estimate', 'Generated points'])
plt.show()
```
