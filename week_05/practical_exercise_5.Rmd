...
model_5.2d <- glmer(correct ~ target.frames + (1 + target.frames | subject), 
                    family = 'binomial', 
                    data = df)
...
anova(model_5.2d, model_5.2c, model_5.2b, model_5.2a)
```
    Correlation between fixed effects:
```{r}
VarCorr(model_5.2d)

ranef <- ranef(model_5.2d) # extract random effects from chosen model and assign to variable

for (i in seq(ranef)){
  assign(paste0("ranef", i), ranef[[i]])
}

colnames(ranef1) <- c("Intercept", "target.frames")

#We now plot the correlation between the random effects of intercept and slope.
plot(ranef1$Intercept, ranef1$target.frames, ylab = "target.frames", xlab = "intercept", main = "Correlation between the random effects of the chosen model")
```
    As the above plot of the correlation of the random effects shows, the effects are negatively correlated with each other. In other words, this means that when the subject's intercept is below the sample mean, their associated slope is 'steeper', while the opposite applies for subjects' whose intercepts are above the sample mean.
    ...
    We chose model_5.2d in order to best describe the probability of obtaining a correct answer in the task when taking into account the number of target frames presented, as well as unique intercepts by subject and unique slopes by target frames presented:
    _correct ~ target.frames + (1 + target.frames | subject)_
    The chosen model performs better than the other models in terms of AIC scores. When conducting a likelihood ratio test, we found the added complexity to significantly improve log-likelihood scores (χ² (2) = 346.41, p < .001). 
```{r}
summary(model_5.2d)
inv.logit(0.83316)
```
    Accuracy is found to be significantly modulated by the number of target frames (beta = 0.83316, SE = 0.04432, p < .001); with each increase in the number of target frames, the probability of a correct answer increases by 69.7%.
```{r}
...
```
    ...
    As seen in the above plot, the estimated function for subject 24 appears to differ from the other subjects, showing an apparently more linear relationship between accuracy and the number of target frames. They appear to demonstrate lower accuracy compared to the other subjects.
```{r}
...
#BUT IS 50% ENOUGH AS CHANCE, SHOULD IT BE 55%??
```
    When running a binomial test on the data of subject 24, we found that their performance, albeit poor, to be significantly better than chance when the level of chance is determined to be a 50% success rate.
...
```{r}
df <- cbind(df, fitted_f = fitted(model_5.2f), probabilities_f = inv.logit(fitted(model_5.2f)))
ggplot(df, aes(x = target.frames, y = probabilities_f, color = subject)) + 
  ...
  theme_minimal()
```
    When participants reported low perceptual awareness (PAS = 1), there does not seem to be any systematic effect on accuracy. A clearer function (approximating a logarithmic curve) emerges as perceptual awareness increases, with less differentiation between subjects.
```{r}
# Test log-likelihood ratios
anova(model_5.2f, model_5.2e, model_5.2d)
```
    The chosen model performs better than the other models in terms of AIC scores. When conducting a likelihood ratio test, we found the added complexity to significantly improve log-likelihood scores (χ² (3) = 379.58, p < .001).
```{r}
summary(model_5.2f)
```
    Accuracy is found to be significantly modulated by the number of target frames (beta = 0.11480, SE = 0.03707, p < .05) and Pas-rating(PAS 2: beta = -0.57139, SE = 0.03707, p < .05; PAS 3: beta = 0.11480, SE = 0.03707, p < .05; PAS 4: beta = 0.11480, SE = 0.03707, p < .05), as well as the interaction of these variables
    This means that accuracy increases 
```{r}
inv.logit(-0.12163) #intercept for PAS 1
inv.logit(-0.12163 + -0.57139) #intercept for PAS 2
inv.logit(-0.12163 + -0.53850) #intercept for PAS 3
inv.logit(-0.12163 + 0.20159) #intercept for PAS 4
```
    According to the model, when 0 target frames are shown, at PAS-rating 1 accuracy is estimated at 47%, at PAS-rating 2 at 33%, at PAS-rating 3 at 34% and at PAS-rating as 52%. This is not reasonable as it makes no sense to consider perceptual accuracy when no stimulus is perceived, and the model does not take into account that when the participant has two options to choose from and no information, their performance will conceptually be at the level of chance, i.e. at 50% accuracy. While the intercept at PAS-rating 1 and 4 are close to this value, this is due to coincidence rather than use of the aforementioned logic.
...
```{r}
#Estimate parameters at for all subjects at all PAS-ratings
#get y_hat values (by plugging in the optimized parameters)
y_hat <- sigmoid_y_hat(df_1, optim_1$par)
df_1 <- cbind(df_1, y_hat)
...
ggplot(df_all, aes(x = target.frames, y = y_hat, color = pas)) + 
  geom_line() +
  xlim(0, 8) +
  labs(x = "Number of target frames",
       y = "Probability of correct answer",
...
for (i in 1:29){
  ...
  # Take 1 subject at a time
  by_subject <- df %>% filter(subject == i)
  data <- data.frame()
  
  for (j in 1:4){
    
    # Take 1 PAS-rating at a time
    by_pas <- by_subject %>% filter(pas == j)
    
    # Optimize parameters for this subject & PAS-rating
    optim <- optim(par = c(0.5, 1, 1, 1),
      fn = RSS, 
      data = by_pas,
      method = 'L-BFGS-B',
      lower = c(0.5, 0.5, -Inf, -Inf),
    ...
    # Data for this subject & PAS-rating
    temp <- as.data.frame(a = optim$par[1],
                          b = optim$par[2],
                          c = optim$par[3],
                          d = optim$par[4],
                          pas = j,
