...
sub24 <- df %>% 
  filter(subject == 24) %>% 
  mutate(correct_num = as.numeric(correct) - 1)
t.test(sub24$correct_num, mu = 0.5)
##### REFORMULATE INTO A REGRESSION?????? We would use the ground truth values because those are not influenced by the other participants' performance as the fitted values are.
```
